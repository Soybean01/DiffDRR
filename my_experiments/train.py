...
def load(id_number, phase_number, height, device):
    datadir = '/home/tianyabin/Project/IM/dataset/4DCT/' + id_number + '/' + phase_number
...
def train(
    ...
    double = DoubleGeodesicSE3(drr.detector.sdd)                    # 几何学2Loss
    ...
    for epoch in range(n_epochs + 1):
        ...
        for _ in (itr := tqdm(range(n_batches_per_epoch), leave=False)):
            ...
            if loss.isnan().any():
                ...
                torch.save(
                    ...
                    f"{save_path}/checkpoints/specimen_{id_number}_{phase_number}_crashed.ckpt",
                )
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"{save_path}/checkpoints/specimen_{id_number}_{phase_number}_best.ckpt",
            )
        ...
        if epoch % 50 == 0:
            torch.save(
                ...
                f"{save_path}/checkpoints/specimen_{id_number}_{phase_number}_epoch{epoch:03d}.ckpt",
            )
            
        loss_save.append(losses.mean().item())
        if epoch % 10 == 0:
            with open(f'{save_path}/losses/loss_{id_number}_{phase_number}.csv', "a") as f:
...
def main(
    ...
    specimen, isocenter_pose, transforms, drr = load(id_number, phase_number, height, device)
...
if __name__ == "__main__":
    ...
    for i in id_numbers:
        for j in phase:
            save_path = '/home/tianyabin/Project/IM/DiffDRR/my_experiments/logs/4DCT_VIT_no_pretrained'
            main(i, j, n_epochs=10000 ,height=224, model_name='vit_base_patch16_224', device=device, save_path=save_path)
